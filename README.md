...
```
#### [2017-04-26]增加自定义toolbar标题&支持对字体的操作
```
...
```
#### [2017-09-05]新增moveToControlPosition 方法，定位到指定的控件
```
因为页面可能存在多个id相同的控件，所以可以多次调用，定位到需要的位置。
```
#### [2017-09-26]修改原有的toolbar扩展方法，支持在原有toolbar的tab中继续新增控件
```
...
```
#### [2017-10-12]实现checkControl方法的正则校验
```
sde.checkControl({
	id:"可选，如果不填就是校验所有控件！"
